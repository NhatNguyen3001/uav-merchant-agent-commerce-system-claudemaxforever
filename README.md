...
Stack: FastAPI, LangGraph, FastMCP, Anthropic API (Claude Opus 5 by default, `MACS_MODEL` selects another model; forced tool calls validated by Pydantic),
...
Anthropic API (Claude Opus 5 by default; other models selectable via `MACS_MODEL`) for the intent decoder, proposal engine and simulated buyer agent;
